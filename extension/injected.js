 ...
 */
(function () {
  if (window.__flowkitMainWorldInitialized) return;
  window.__flowkitMainWorldInitialized = true;
  window.__flowkitSiteKey = window.__flowkitSiteKey || '6LdsFiUsAAAAAIjVDZcuLhaHiDn5nnHVXVRQGeMV';
  ...
  window.addEventListener('GET_CAPTCHA', async ({ detail }) => {
    ...
    try {
      ...
      const token = await window.grecaptcha.enterprise.execute(window.__flowkitSiteKey, {
        action: pageAction,
      });
