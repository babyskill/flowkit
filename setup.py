...
Usage:
    ...
    python setup.py --tool trae      # Generate for Trae (.trae/skills)
...
def discover_skills():
    """Scan skills/fk-*.md (and legacy skills/fk:*.md) and extract name + first-line description."""
    if not SKILLS_DIR.exists():
        print(f"ERROR: skills/ directory not found at {SKILLS_DIR}")
        return []

    patterns = ["fk-*.md", "fk:*.md"]
    paths = []
    ...
    for path in sorted(set(paths)):
        ...
        try:
            ...
            first_line = _first_nonempty_line(raw)
            description = first_line.lstrip("#").strip() or f"{stem} skill"
        except OSError:
            description = f"{stem} skill"

        skills.append({"name": stem, "description": description, "path": str(path)})

    return skills


def generate_claude(skills):
    """Create .claude/commands/<skill>.md stubs."""
    CLAUDE_COMMANDS_DIR.mkdir(parents=True, exist_ok=True)
    count = 0
    for skill in skills:
        name = skill["name"]
        desc = skill["description"]
        dest = CLAUDE_COMMANDS_DIR / f"{name}.md"
        content = (
            f"{desc}\n"
            f"<!-- AUTO-GENERATED by setup.py — do not edit. Source: skills/ -->\n"
            f"Read and follow the instructions in skills/{name}.md\n\n"
...
def generate_gemini(skills):
    ...
    for skill in skills:
        ...
        content = (
            f'# AUTO-GENERATED by setup.py — do not edit. Source: skills/\n'
            f'description = "{desc}"\n'
            f'prompt = "Read the skill file at skills/{name}.md and follow the instructions inside. Arguments: $ARGUMENTS"\n'
        )
        dest.write_text(content, encoding="utf-8")
        count += 1
...
def _agents_md_body(skills, title):
    ...
    return (
        ...
        f"{_BATCH_API}\n"
        f"## Skills\n\n"
        f"This project has reusable skills in `skills/`. When the user says `/<skill>`, "
        f"read `skills/<skill>.md` and follow the instructions inside.\n\n"
        f"| Skill | Purpose |\n"
        f"|-------|---------|\n"
        f"{rows}\n"
    )
...
def generate_trae(skills):
    """Generate .trae/skills/<skill>/SKILL.md for Trae."""
    ...
    print(f"Generated {count} Trae skills in .trae/skills/")
    return count
...
def do_interactive(skills):
    ...
    mapping = {"1": "claude", "2": "gemini", "3": "codex", "4": "trae", "5": "all"}
    ...
    for c in choices:
        ...
        val = mapping[c]
        if val == "all":
            tools = ["claude", "gemini", "codex", "trae"]
            break
        if val not in tools:
            tools.append(val)
...
def do_clean():
    ...
    # Trae skills (only fk-* subdirs)
    if TRAE_SKILLS_DIR.exists():
        for d in sorted(TRAE_SKILLS_DIR.glob("fk-*")):
            skill_md = d / "SKILL.md"
            if skill_md.exists():
                skill_md.unlink()
                removed.append(str(skill_md))
            try:
                d.rmdir()
            except OSError:
                pass  # leave non-empty dirs intact
...
def main():
    ...
    parser.add_argument(
        "--tool",
        choices=["claude", "gemini", "codex", "trae", "all"],
